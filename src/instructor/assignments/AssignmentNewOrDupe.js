...
function AssignmentNewOrDupe() {
  ...
  async function fetchAssignmentList() {
    ...
    try {
      ...
      reportError(
        error,
        `We're sorry. There was an error while attempting to fetch the list of your existing assignments for duplication.`
      );
    }
  }
    ...
    try {
      const inputData = Object.assign({}, selectedAssignment, {
        title: !selectedAssignment.lineItemId
          ...
          : `Copy of ${selectedAssignment.title}`,
        lineItemId: "",
        isLinkedToLms: false,
      ...
      if (selectedAssignment.lineItemId) {
        ...
        setActiveModal({
          ...
          data: [selectedAssignment.title, result.data.createAssignment],
        });
      } else {
        result = await API.graphql({
        ...
        setActiveModal({
          ...
          data: [selectedAssignment.title, result.data.updateAssignment],
        });
      }
    } catch (error) {
      reportError(
        error,
        `We're sorry. There was a problem duplicating and saving your new assignment.`
      );
    }
  }
    ...
    switch (activeModal.type) {
        ...
        return (
          ...
          >
            {activeModal.data[0].lineItemId ? (
              <p>
                A new assignment called Copy of {activeModal.data[0]} has been
                saved! It is now accessible in your LMS.
              </p>
            ) : (
              <p>
                You will now be taken to a screen so you can edit and customize
                your newly duplicated assignment.
              </p>
            )}
          </ConfirmationModal>
        );
      case MODAL_TYPES.confirmAssignmentRecovered:
        return (
          ...
          >
            <p>
              Your assignment "{activeModal.data[0]}" has been recovered. You
              will now be taken to a screen so you can edit and customize this
